...
Python 3.12 est conseillé pour rester cohérent avec l'environnement indiqué par les principaux notebooks.

```bash
git clone https://github.com/Georges-Imaho/deep-hedging-pytorch.git
cd deep-hedging-pytorch
python -m venv .venv
```
...
Installation des dépendances directes :
...
L'exemple suivant calcule un prix Black--Scholes et simule des trajectoires GBM. Il ne lance pas l'entraînement du LSTM.
...
price = BlackScholesOracle.get_price(
    S=100.0,
    K=100.0,
    T=1.0,
    r=0.02,
...
simulator = MarketSimulator(s0=100.0, r=0.02, sigma=0.20, dt=1 / 252)
paths = simulator.simulate_gbm(steps=252, n_paths=1_000)

print(f"Prix du call : {price:.4f}")
print(f"Forme des trajectoires : {paths.shape}")  # (253, 1000)
```

## Tests

Le dépôt ne contient pas encore de suite de tests automatisés. Le contrôle suivant constitue seulement un smoke test d'import et de calcul ; il ne valide pas scientifiquement les modèles :

```bash
python -c "from src.analytics_models import BlackScholesOracle; p = BlackScholesOracle.get_price(100, 100, 1, 0.02, 0.2); assert p > 0; print('Smoke test OK')"
```

La commande cible, une fois un dossier `tests/` ajouté, sera :

```bash
python -m pytest
```

Les tests prioritaires concernent les cas limites Black--Scholes, les moments du GBM, la discrétisation Heston, les dimensions du LSTM et le calcul manuel du PnL sur des trajectoires simples.

## État actuel du projet

| Composant | État |
|---|---|
| Prix et delta Black--Scholes | Implémentés, sans tests unitaires systématiques |
| Simulation GBM | Implémentée et illustrée dans les notebooks |
| Simulation Heston | Implémentation Euler exploratoire, validation numérique incomplète |
| Politique LSTM | Architecture implémentée, entraînement encore expérimental |
| PnL et coûts proportionnels | Implémentés sous une convention simplifiée |
| Benchmark LSTM / delta Black--Scholes | Notebook présent, résultats non validés |
| Tests automatisés | Absents |

Certaines sorties historiques de `test_LSTM.ipynb` contiennent des valeurs non finies. Elles ne doivent pas être interprétées comme des résultats expérimentaux. Aucun avantage du LSTM par rapport à Black--Scholes n'est revendiqué.

## Limites méthodologiques

- La cohérence entre `T`, `dt` et le nombre de pas doit être imposée explicitement dans les expériences.
- La volatilité brute et sa version standardisée doivent rester distinctes lors du pricing et de la construction des features.
- La discrétisation Heston peut produire des valeurs de variance négatives avant correction numérique et n'a pas fait l'objet d'une étude de convergence.
- La volatilité instantanée simulée est traitée comme observable dans le notebook, alors qu'elle est latente en pratique.
- Le PnL ne modélise pas encore complètement le financement et la liquidation terminale.
- Les remplacements de valeurs non finies utilisés dans le prototype peuvent masquer une erreur en amont et doivent être remplacés par des diagnostics explicites.
- L'entraînement, la validation et le test ne sont pas organisés en expériences reproductibles avec plusieurs seeds.
- Les paramètres Heston et les coûts de transaction ne sont pas calibrés sur des données empiriques.
- Aucun intervalle de confiance ni test statistique n'accompagne les sorties actuelles.

## Prochaines étapes

1. ajouter des tests unitaires pour Black--Scholes, GBM, Heston et le PnL ;
2. imposer une grille temporelle cohérente entre simulation, maturité et rebalancement ;
3. séparer explicitement volatilité brute, volatilité normalisée et volatilité observable ;
4. documenter puis tester une convention de portefeuille autofinancé avec liquidation finale ;
5. fiabiliser la gestion des NaN et rendre les expériences reproductibles ;
6. constituer des jeux de simulation indépendants pour entraînement, validation et test ;
7. comparer le LSTM à la couverture delta analytique sur les mêmes trajectoires ;
8. rapporter erreur moyenne, dispersion, quantiles de perte et intervalles de confiance ;
9. étudier ensuite une calibration sur données de marché, si elle devient pertinente pour le projet de thèse.
